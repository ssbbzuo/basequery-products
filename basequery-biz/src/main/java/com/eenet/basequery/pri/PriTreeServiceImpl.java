...
public class PriTreeServiceImpl extends SimpleBizImpl implements PriTreeService   {
		...
		try {
			switch (type) {
				...
				case 4:
					...
					return resultSet;
				case 5:
					cache  =  RedisClient.getObject("PRI_TREE"+PrivilegeType.LEARNCENTERPRI.name()+id, SimpleResultSet.class);
					if (cache!=null) {
						return cache;
					}
	...
	@Override
	public List<String> getPriTreeByType(String uid,int type){
		switch (type) {
			case 1:
				return handlePriTree(uid, PrivilegeType.AREAPRI) ; 
			case 2:
				return handlePriTree(uid, PrivilegeType.NETWORKPRI) ;  
			case 3:
				return handlePriTree(uid, PrivilegeType.STAFFPRI) ; 
			case 4:
				return handlePriTree(uid, PrivilegeType.CHANELPRI) ; 
			case 5:
				
				return handlePriTree(uid, PrivilegeType.LEARNCENTERPRI) ; 
			default:
				return null;
		}
	}
	...
	private void removeCachedPriTree(String uid,String pId ,String removeIds,int type){
		System.out.println("removeCachedPriTree :　uid：" +uid +",pId:" +pId +",removeIds:" +removeIds+",type:" +type);
		try {
			List<String> cache  =  RedisClient.getObject("PRI_TREE"+uid+pId+type, List.class);
